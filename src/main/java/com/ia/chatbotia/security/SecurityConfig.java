...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.csrf().disable()
                .authorizeRequests()
                .antMatchers("/empleados/save").permitAll() // Permite acceso a /usuario/save sin autenticación
                .antMatchers("/swagger-ui/index.html", "/v3/api-docs/**", "/swagger-ui/**").permitAll() // Permite acceso a Swagger sin autenticación
                .antMatchers("/empleados/login").permitAll() // Permite acceso a /usuario/login sin autenticación
